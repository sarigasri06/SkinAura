...
const categoryMeta = {
  cleanser:    { label: 'Cleansers',        icon: FiDroplet,   color: 'from-rose-300 to-rose-500',   desc: 'Gentle & effective face washes' },
  moisturizer: { label: 'Moisturizers',     icon: FiShield,    color: 'from-rose-300 to-rose-500', desc: 'Deep hydration for every skin type' },
  serum:       { label: 'Serums',           icon: FiPlus,      color: 'from-rose-300 to-rose-500', desc: 'Targeted treatments & actives' },
  toner:       { label: 'Toners',           icon: FiGrid,      color: 'from-rose-300 to-rose-500',    desc: 'Balance & prep your skin' },
  sunscreen:   { label: 'Sunscreens',       icon: FiSun,       color: 'from-rose-300 to-rose-500', desc: 'Daily UV protection' },
  mask:        { label: 'Masks',            icon: FiClipboard, color: 'from-rose-300 to-rose-500',  desc: 'Deep treatments & pampering' },
  'eye-care':  { label: 'Eye Care',         icon: FiEye,       color: 'from-rose-300 to-rose-500',    desc: 'Brighten & depuff' },
  'lip-care':  { label: 'Lip Care',         icon: FiCoffee,    color: 'from-rose-300 to-rose-500',    desc: 'Nourish & protect lips' },
  'body-care': { label: 'Body Care',        icon: FiGlobe,     color: 'from-rose-300 to-rose-500',   desc: 'Full body skincare' },
  'face-care': { label: 'Face Care',        icon: FiShield,    color: 'from-rose-300 to-rose-500', desc: 'Oils, mists & treatments' },
  set:         { label: 'Sets & Bundles',   icon: FiGift,      color: 'from-rose-300 to-rose-500', desc: 'Curated routine bundles' },
};
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="text-center mb-12">
        <h1 className="text-3xl md:text-4xl font-serif font-bold text-rose-900 mb-4">
          Shop by Category
        </h1>
      ...
      {categoryLoading ? (
            ...
            <section key={slug} id={slug} className="scroll-mt-24">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-rose-500">{meta.desc}</p>
                </div>
                <Link
        ...
        <div className="text-center py-20">
          ...
          <p className="text-rose-500">Check back soon for new arrivals!</p>
        </div>
      )}
      ...
      {categoriesWithProducts.length > 0 && (
        <div className="fixed right-4 bottom-20 z-40 hidden xl:block">
          <div className="bg-white rounded-2xl shadow-lg border border-rose-100 p-3 space-y-1">
            <p className="text-xs font-semibold text-rose-400 uppercase tracking-wider px-2 mb-2">
              Jump to
            </p>
            {categoriesWithProducts.map(([slug, meta]) => (
              <a
                key={slug}
                href={`#${slug}`}
                className="flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm text-rose-600 hover:bg-rose-50 hover:text-rose-600 transition-colors"
              >
